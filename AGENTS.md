...
| Aspect | Decision |
|---|---|
| Input | **Five steps plus a review screen**, in one `<form>` in `components/intake/IntakeForm.tsx`, which owns all the state so going back never loses an answer. Step 1 setup (attachments · project type · tone · title · client_type · year & duration · role · team/credit · `voice_sample`) · Step 2 the problem (what was wrong · why it mattered · what constrained you) · Step 3 **the decisions** · Step 4 your screens, *shown only if something was uploaded* · Step 5 where it landed · Review. **Next is never blocked** — every answer may be empty all the way to submit, and the review screen points at what is thin instead. There is no separate "Client or company" field: `client_type` fills the CLIENT slot in the metadata grid, the way the proven fixture does. Step 1 is now the heaviest step and still scrolls; the other four are far lighter, which is all that is claimed. |
